 ...
 *   formálisan eltűnik az első paraméterük (s0) (ezt this-ként azért
 * megkapják). A nevük is megváltozik, mert most már az összetartozást, és a
...
/**
 * Az Ön feladata ebben fájlban bővíteni a megkezdett deklarációt, és
 * string5.cpp fájlban pedig megvalósítani (definiálni) az osztály egyes
 * függvényeit úgy, hogy az a lenti követelményeknek eleget tegyen.
 *
...
/**
 * \section fel Feladat:
 * A 2. laborgyakorlaton létrehoztunk egy dinamikus sztring (String)
 *adatszerkezet és a rajta műveleteket végző függvényeket A tanultakat
 *felhasználva a string5.h és a string5.cpp állományokban definiáljon ill.
 * implementáljon egy olyan sztring (String) osztályt C++ nyelven, ami
 ...
 *  2. Van olyan tagfüggvénye ( c_str() ), ami C-sztringgel, azaz nullával
 *lezárt karaktersorozatra mutató pointerrel (const char *) tér vissza.
 *  3. Van olyan konstruktora, ami karakterből hoz létre sztringet.
 *  4. Van olyan konstruktora, ami C-sztringből (const char*) hoz létre
 *sztringet.
 ...
 * 10. Készítsen olyan << operátort, amivel egy sztring kiírható egy ostream
 *típusú objektumra!
 *
 * Szorgalmi feladatok:
 * 11. Készítsen olyan fűzhető >> operátort, amivel be tud olvasni egy szót egy
 *sztring-be! A beolvasó működjön úgy, mint a scanf %s, azaz a szó végét white
 *space határolja! A szó eleji white space karaktereket pedig el kell dobni.
 *    *Megj: az istream wshite space kezelése a flags() tagfüggvénnyel
 *szabályozható.
 * 12. Próbálja ki ékezetes szövegekkel is az elkészített String osztályt!
 *     Segítségül elkészítettünk egy egyszerű kódrészletet ami szavakat olvas
 *be, majd qsort() függvénnyel rendezi azokat. Az ékezetes karakterek
 *kódolásával kapcsolatban korábban már volt egy egyszerű szorgalmi feladat
 *feladat. Most a rendezésen van a hangsúly. Értse meg a rendez.cpp fájlban levő
 *kód működését és futtassa a programot!
 * 13. A setlocale függvényhívás beállítja a karakterek és sztringek
 *összehasonlításához szükséges nyelvi környezetet (LC_COLLATE) próbálja ki!
 *Változott a rendezési sorrend?
 */
 ...
 *
 * Korábban a len adattag unsigned int típusú volt. Most helyette a size_t
 típust használjuk.
 * Ezzel a típussal tér vissza a sizeof operator is, később több helyen ezt
 használjuk
 * mérettípusként.
...
class String {
    ...
    String(size_t len) : len(len) { pData = new char[len + 1]; }
  ...
  public:
    ...
    String() : String((size_t)0) { pData[0] = 0; }
    ...
    /// @return pointer a tárolt, vagy azzal azonos tartalmú nullával lezárt
    /// sztring-re.

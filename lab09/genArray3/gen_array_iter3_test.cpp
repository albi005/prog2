...
#ifndef ELKESZULT
#define ELKESZULT 15 /// ha mégse definiálta
#endif
...
int main() {
    ...
    try {
        ...
        TEST(PrintArray, adat) {
            ...
            EXPECT_EQ(
                std::string("cpp size=3 capacity=10\n\t data=C,+,+\n"),
                out.str()
        ...
        i1 = i2 = intarr0.begin();
        if (i1 != i2++) {
            cout << "** HIBAS az iterator post-inkremens operatora **\n";
        }
#endif // ELKESZULT >= 13
        ...
        int db = szamol_ha(intarr1.begin(), intarr1.end(), [](int x) {
            return x > 5;
        });
        // db = szamol_ha(intarr1.begin(), intarr1.end(), nagyobb_mint<int>(5));
    ...
    } catch (std::exception& e) { // az out_of_range, és a runtime_error is az
                                  // exception osztályból származik. Így azok
                                  // kompatibilisek az alaposztállyal
        cout << e.what() << endl;
    }
}

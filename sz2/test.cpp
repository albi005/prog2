 ...
 * A szorgalmi feladat megoldásához ezt az állományt nem kell beadni
 * (feltölteni).
 *
 * A ELKESZULT makró vezérli az egyes tesztesetek fordítását, ezzel lehetővé
 * válik hogy kisebb lépésekben teszteljünk.
 *
 */
...
#if ELKESZULT > 8
    TEST(Cipher_Queue
         : caesar + mycipher, _meg tobb szorgalmit prog2bol) {
        CipherQueue queue;
...
#if ELKESZULT > 9
    TEST(Cipher_Queue
         : inner queue, _pointert mindenhova_beta) {

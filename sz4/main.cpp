...
template <typename T> void runstr(T& allapotgep, std::string str) {
    for (char c : str)
        allapotgep(c);
}
...
int main() {
    ...
    END TEST(Komment, latin) {
        Komment k;
        runstr(
            k,
            "/*c++**/*Ad maiorem Dei gloriam/**:)(*/ Alea iacta est /**/Et tu "
            "mi fili, Brute?"
        );
        EXPECT_STREQ(
            "*Ad maiorem Dei gloriam Alea iacta est Et tu mi fili, Brute?",
            k.get().c_str()
        );
    }
    END
